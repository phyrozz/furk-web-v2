...
const AffiliateDetails: React.FC<AffiliateDetailsProps> = ({ affiliate, onStatusChange }) => {
...
const onApprove = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      ToastService.show('Failed to approve affiliate');
    }
  } catch (error) {
    console.error('Error approving affiliate:', error);
    ToastService.show('Error approving affiliate');
  } finally {
  ...
  const onReject = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        ToastService.show('Failed to reject affiliate');
      }
    } catch (error) {
      console.error('Error rejecting affiliate:', error);
      ToastService.show('Error rejecting affiliate');
    } finally {
    ...
    <div className="bg-white rounded-lg shadow">
      ...
      <div className="p-6 border-b">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className={`px-3 py-1 rounded-full border ${getStatusBadgeColor(affiliate.application_status)}`}>
            {getStatusDisplayText(affiliate.application_status)}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 mt-2">
          ...
          <div className="text-right">
            <p className="text-gray-500 text-sm">
              Created: {new Date(affiliate.created_at).toLocaleString()}
            </p>
            {affiliate.modified_at && (
              <p className="text-gray-500 text-sm mt-1">
                Last Updated: {new Date(affiliate.modified_at).toLocaleString()}
              </p>
            )}
          </div>
        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="mt-1">{affiliate.username}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-500">
                    Contact Number
                  </label>
                  <p className="mt-1">{affiliate.phone_number}</p>
                </div>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to approve ${affiliate.first_name} ${affiliate.last_name}? This will enable their affiliate ID for merchants to use.`}
        confirmText="Approve"
        cancelText="Cancel"
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to reject ${affiliate.first_name} ${affiliate.last_name}? They will need to reapply to join the platform.`}
        confirmText="Reject"
        cancelText="Cancel"
