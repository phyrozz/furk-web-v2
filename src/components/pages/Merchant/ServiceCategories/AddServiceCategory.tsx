...
const AddServiceCategoryForm: React.FC<AddServiceCategoryFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: name === 'service_group_id' ? parseInt(value) || 0 : value
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await http.post('/service-categories/insert', formData);
      onSuccess();
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="p-6 border-b">
        <h2 className="text-xl font-semibold text-gray-800">Add New Service Category</h2>
        <p className="text-sm text-gray-500 mt-1">
          Create a new service category for your business
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        ...
        <div>
          ...
          <Autocomplete
            ...
            onChange={(option) => setFormData({ ...formData, service_group_id: option?.id || 0 })}
